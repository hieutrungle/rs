...
class Conference4UEAllocation(EnvBase):
    metadata = {
    ...
    def __init__(
        ...
        *,
        ...
        self.rx_polygon_coords = [
            [(-0.3, 2.0), (-4.0, -2.0), (-4.0, -5.5), (0.0, -5.5), (2.7, 2.1)],
    ...
    def _reset(self, tensordict: TensorDict = None) -> TensorDict:
        ...
        if self.no_allocator or self.random_assignment:
            ...
            # Select locations for each Reflector agent
        ...
        non_overlapping_indices = len(set(self.selected_loc_indices.tolist()))
        if non_overlapping_indices < self.num_rf:
            self.allocator_reward_const = torch.ones((1, 1), device=self.device) * (-10.0) / 30.0
            self.allocator_reward_const *= self.num_rf - non_overlapping_indices
        else:
            self.allocator_reward_const = torch.ones((1, 1), device=self.device) * 10.0 / 30.0
