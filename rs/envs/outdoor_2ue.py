...
class Outdoor2UEAllocation(EnvBase):
    metadata = {
    ...
    def __init__(
        ...
        *,
        ...
        self.init_focals = torch.tensor(
            [[174.546, 131.649, 1.5] for _ in range(self.num_rf)],
            dtype=torch.float32,
            device=device,
        )
        self.init_focals = self.init_focals.unsqueeze(0)
        self.focal_low = torch.tensor(
            [[[105.766, 60.8472, -4.0], [105.766, 60.8472, -4.0]]], device=device
        )
        self.focal_high = torch.tensor(
            [[[227.491, 181.943, 5.0], [227.491, 181.943, 5.0]]], device=device
        )
        ...
        rx_low = torch.ones_like(rx_positions, device=device) * (77.6526)
        rx_high = torch.ones_like(rx_positions, device=device) * 272.357
        rf_low = torch.ones_like(rf_positions, device=device) * (77.6526)
        rf_high = torch.ones_like(rf_positions, device=device) * 272.357
        ...
        self.rx_polygon_coords = [
            [(142.723, 158.538), (203.03, 156.752), (204.488, 97.3238), [139.104, 93.3212]],
            [(142.723, 158.538), (203.03, 156.752), (204.488, 97.3238), [139.104, 93.3212]],
        ]
    ...
    def _generate_moved_rx_positions(self, pos: np.ndarray, polygon: Polygon) -> Point:
        ...
        """
        r = 3  # radius of the circle to move the position
        while True:
    ...
    def _normalize_allocation_states(self):
        ...
        """
        shape = self.allocation_agent_states.shape
        loc = torch.zeros(shape, device=self.device)
        scale = torch.ones(shape, device=self.device)
        loc[..., 0] = loc[..., 0] + 178.777
        loc[..., 1] = loc[..., 1] + 213.079
        loc[..., 3] = loc[..., 3] + 174.546
        loc[..., 4] = loc[..., 4] + 130.649
        scale[..., 0] = scale[..., 0] * 30.0  # rf x position
        scale[..., 1] = scale[..., 1] * 10.0  # rf y position
        scale[..., 2] = scale[..., 2] * 9.0  # rf z position
        scale[..., 3] = scale[..., 3] * 40.0  # focal x position
        scale[..., 4] = scale[..., 4] * 40.0  # focal y position
        scale[..., 5] = scale[..., 5] * 5.0  # focal z position
        self.allocation_agent_states = (self.allocation_agent_states - loc) / scale  # rf, focals
        self.allocation_init_agent_states = (self.allocation_init_agent_states - loc) / scale

        shape = self.allocation_target_states.shape
        loc = torch.zeros(shape, device=self.device)
        scale = torch.ones(shape, device=self.device)
        loc[..., 0] = loc[..., 0] + 174.546
        loc[..., 1] = loc[..., 1] + 130.649
        ...
        self.allocation_target_states = (self.allocation_target_states - loc) / scale
    ...
    def _reset(self, tensordict: TensorDict = None) -> TensorDict:
        ...
        if self.focals is None or not self.eval_mode:
            ...
            delta_focals[..., 0] = delta_focals[..., 0] * 18.0
            delta_focals[..., 1] = delta_focals[..., 1] * 18.0
            delta_focals[..., 2] = delta_focals[..., 2] * 0.0
    ...
    def _step(self, tensordict: TensorDict) -> TensorDict:
        ...
        delta_focals[..., 0] = delta_focals[..., 0] * 3.0
        delta_focals[..., 1] = delta_focals[..., 1] * 3.0
        delta_focals[..., 2] = delta_focals[..., 2] * 0.0
        # delta_focals[..., 2] = delta_focals[..., 2] * 0.4
