...
#         return cur_rss[:, 0, 0, :].detach().cpu().numpy(), list(
#             cur_rss[:, 0, 0, :].detach().cpu().numpy()
#         )
...
#             current_best = np.array([-np.inf] * self.n_agents)
#             for ind, r in zip(pop, rss):
...
#         self.best_fitness = np.array([-np.inf] * self.n_agents)
#         self.best_rss = None
...
def main(config: TrainConfig):
    ...
    focal_lows[..., 2] = 0.5  # z-coordinate should be 0.0
    ...
    focal_highs[..., 2] = 2.5  # z-coordinate should be 0.0
